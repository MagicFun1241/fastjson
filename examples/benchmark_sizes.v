...
fn bench_decode[T](label string, n int, pool []string) time.Duration {
...
fn bench_roundtrip[T](label string, n int, pool []string) time.Duration {
	mut sw := time.new_stopwatch()
	sw.start()
	for i := 0; i < n; i++ {
		input := fastjson.decode[T](pool[i % pool_size]) or { panic(err) }
...
fn print_bench(name string, d time.Duration, n int) {
...
fn main() {
	...
	// Warmup
	bench_decode[SmallStruct]('warmup', 5000, small_pool)
	bench_decode[MediumStruct]('warmup', 5000, medium_pool)
	bench_decode[LargeStruct]('warmup', 5000, large_pool)

	println('--- Small (~${small_size}B, 11 keys) ---')
	print_bench('decode', bench_decode[SmallStruct]('small_decode', iterations, small_pool), iterations)
	print_bench('encode', bench_encode[SmallStruct]('small_encode', iterations, small_pool), iterations)
	print_bench('roundtrip', bench_roundtrip[SmallStruct]('small_roundtrip', iterations, small_pool), iterations)
	println('')

	println('--- Medium (~${medium_size}B, 13 keys + nested) ---')
	print_bench('decode', bench_decode[MediumStruct]('medium_decode', iterations, medium_pool), iterations)
	print_bench('encode', bench_encode[MediumStruct]('medium_encode', iterations, medium_pool), iterations)
	print_bench('roundtrip', bench_roundtrip[MediumStruct]('medium_roundtrip', iterations, medium_pool), iterations)
	println('')

	println('--- Large (~${large_size}B, 9 keys + big arrays/maps) ---')
	print_bench('decode', bench_decode[LargeStruct]('large_decode', iterations, large_pool), iterations)
	print_bench('encode', bench_encode[LargeStruct]('large_encode', iterations, large_pool), iterations)
	print_bench('roundtrip', bench_roundtrip[LargeStruct]('large_roundtrip', iterations, large_pool), iterations)
	println('')
